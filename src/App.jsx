...
const NAV = [
  ...
  {id:"richieste",icon:"📦",label:"Richieste/Consumi"},
];
        ...
        <div style={{ display:"flex", minHeight:"100vh", background:t.bg, fontFamily:"'DM Sans',sans-serif", fontSize:"1rem" }}>
          ...
          <main id="main-content" style={{ flex:1, marginLeft:"13.5rem", padding:"1.5rem", overflowY:"auto" }}>
            {!canAccess ? (
              ...
              {activeTab==="richieste"&&<RichiesteConsumi/>}
            </>}
          </main>
...
// ===================== RICHIESTE / CONSUMI =====================
function RichiesteConsumi() {
  const t = useTheme();
  const today = new Date();
  const [workers, setWorkers] = useState([]);
  const [prodotti, setProdotti] = useState([]);
  const [richieste, setRichieste] = useState([]);
  const [view, setView] = useState("dashboard"); // "dashboard" | "richieste" | "prodotti"
  ...
  const blankProd = { name:"", unit:"pz", category:"" };
  const blankReq = { date: today.toISOString().slice(0,10), workerId:"", items:[{prodId:"",qty:""}], notes:"" };
  const [prodForm, setProdForm] = useState(blankProd);
  const [reqForm, setReqForm] = useState(blankReq);

  useEffect(()=>{(async()=>{
    setWorkers(await storage.get("workers")||[]);
    setProdotti(await storage.get("richProdotti")||[]);
    setRichieste(await storage.get("richRichieste")||[]);
  })();},[]);

  const saveProdotti = async p => { setProdotti(p); await storage.set("richProdotti", p); };
  const saveRichieste = async r => { setRichieste(r); await storage.set("richRichieste", r); };

  // --- Prodotti CRUD ---
  const submitProd = async () => {
    if (!prodForm.name) return;
    if (editProd) {
      await saveProdotti(prodotti.map(p => p.id===editProd.id ? {...p,...prodForm} : p));
    } else {
      await saveProdotti([...prodotti, {...prodForm, id: Date.now().toString()}]);
    }
    setProdForm(blankProd); setShowProdForm(false); setEditProd(null);
  };
  const removeProd = async id => {
    if (!confirm("Eliminare questo prodotto?")) return;
    await saveProdotti(prodotti.filter(p => p.id !== id));
  };
  const openEditProd = p => { setProdForm({name:p.name,unit:p.unit,category:p.category||""}); setEditProd(p); setShowProdForm(true); };

  // --- Richieste CRUD ---
  const addReqItem = () => setReqForm({...reqForm, items:[...reqForm.items,{prodId:"",qty:""}]});
  const updReqItem = (i,f,v) => { const items=[...reqForm.items]; items[i][f]=v; setReqForm({...reqForm,items}); };
  const removeReqItem = i => { const items=reqForm.items.filter((_,idx)=>idx!==i); setReqForm({...reqForm, items: items.length?items:[{prodId:"",qty:""}]}); };
  const submitReq = async () => {
    if (!reqForm.workerId) return;
    const items = reqForm.items.filter(i=>i.prodId&&i.qty);
    if (!items.length) return;
    if (editReq) {
      await saveRichieste(richieste.map(r => r.id===editReq.id ? {...r,...reqForm,items} : r));
    } else {
      await saveRichieste([...richieste, {...reqForm, items, id: Date.now().toString(), createdAt: new Date().toISOString()}]);
    }
    setReqForm(blankReq); setShowReqForm(false); setEditReq(null);
  };
  const removeReq = async id => {
    if (!confirm("Eliminare questa richiesta?")) return;
    await saveRichieste(richieste.filter(r => r.id !== id));
  };
  const openEditReq = r => {
    setReqForm({date:r.date,workerId:r.workerId,items:r.items,notes:r.notes||""});
    setEditReq(r); setShowReqForm(true);
  };

  // --- Dashboard data ---
  const monthKey = `${filterYear}-${String(filterMonth+1).padStart(2,"0")}`;
  const monthReqs = richieste.filter(r => r.date && r.date.startsWith(monthKey));

  // For each worker, sum all products requested this month
  const activeWorkers = workers.filter(w => w.active);
  // Build per-worker consumption map: workerId -> { prodId -> totalQty }
  const consumption = {};
  monthReqs.forEach(r => {
    if (!consumption[r.workerId]) consumption[r.workerId] = {};
    r.items.forEach(item => {
      consumption[r.workerId][item.prodId] = (consumption[r.workerId][item.prodId]||0) + parseFloat(item.qty||0);
    });
  });
  // All products used this month
  const usedProdIds = [...new Set(monthReqs.flatMap(r => r.items.map(i=>i.prodId)))];

  const UNITS = ["pz","kg","g","l","ml","conf","rotoli","sacchi","flaconi","altro"];
  const CATEGORIES = ["Pulizie","Biancheria","Cucina","Igiene","Manutenzione","Altro"];

  const ProdForm = (
    <div>
      <FGrid>
        <div><Lbl>Nome prodotto *</Lbl><Inp placeholder="es. Detersivo pavimenti" value={prodForm.name} onChange={e=>setProdForm({...prodForm,name:e.target.value})}/></div>
        <div><Lbl>Unità di misura</Lbl>
          <Sel value={prodForm.unit} onChange={e=>setProdForm({...prodForm,unit:e.target.value})}>
            {UNITS.map(u=><option key={u}>{u}</option>)}
          </Sel>
        </div>
        <div><Lbl>Categoria</Lbl>
          <Sel value={prodForm.category} onChange={e=>setProdForm({...prodForm,category:e.target.value})}>
      ...
      <div style={{ display:"flex",gap:"0.4rem",marginTop:"0.75rem" }}>
        <Btn onClick={submitProd}>{editProd?"Aggiorna":"Salva"}</Btn>
        <Btn variant="secondary" onClick={()=>{setShowProdForm(false);setEditProd(null);setProdForm(blankProd);}}>Annulla</Btn>
      </div>
    </div>
  );
  ...
  const ReqForm = (
    <div>
      <FGrid>
        <div><Lbl>Data *</Lbl><Inp type="date" value={reqForm.date} onChange={e=>setReqForm({...reqForm,date:e.target.value})}/></div>
        <div><Lbl>Lavoratore *</Lbl>
          <Sel value={reqForm.workerId} onChange={e=>setReqForm({...reqForm,workerId:e.target.value})}>
            <option value="">Seleziona...</option>
            {workers.filter(w=>w.active).map(w=><option key={w.id} value={w.id}>{displayName(w)} {w.surname}</option>)}
          </Sel>
        </div>
      </FGrid>
      <div style={{ marginTop:"0.65rem" }}>
        <Lbl>Prodotti richiesti *</Lbl>
        {reqForm.items.map((item,i)=>(
          <div key={i} style={{ display:"flex",gap:"0.4rem",marginBottom:"0.35rem",alignItems:"center" }}>
            <Sel style={{ flex:3 }} value={item.prodId} onChange={e=>updReqItem(i,"prodId",e.target.value)}>
              <option value="">Seleziona prodotto...</option>
              {prodotti.map(p=><option key={p.id} value={p.id}>{p.name} ({p.unit})</option>)}
            </Sel>
            <Inp style={{ flex:1 }} type="number" min="0" step="0.1" placeholder="Qtà" value={item.qty} onChange={e=>updReqItem(i,"qty",e.target.value)}/>
            <IconBtn onClick={()=>removeReqItem(i)} icon="✕" color={t.red} title="Rimuovi"/>
          </div>
        ))}
        {prodotti.length===0 && <p style={{color:t.amber,fontSize:"0.78rem"}}>⚠️ Nessun prodotto disponibile. Aggiungine prima dalla sezione "Prodotti".</p>}
        <Btn variant="secondary" onClick={addReqItem}>+ Prodotto</Btn>
      </div>
      <div style={{ marginTop:"0.5rem" }}><Lbl>Note (opz.)</Lbl><Inp textarea style={{ height:"3rem" }} value={reqForm.notes} onChange={e=>setReqForm({...reqForm,notes:e.target.value})}/></div>
      <div style={{ display:"flex",gap:"0.4rem",marginTop:"0.75rem" }}>
        <Btn onClick={submitReq}>{editReq?"Aggiorna":"Salva"}</Btn>
        <Btn variant="secondary" onClick={()=>{setShowReqForm(false);setEditReq(null);setReqForm(blankReq);}}>Annulla</Btn>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth:"75rem",margin:"0 auto" }}>
      <PageHeader title="📦 Richieste / Consumi">
        <div style={{ display:"flex",gap:"0.4rem",flexWrap:"wrap" }}>
          <Btn onClick={()=>{ setReqForm(blankReq); setEditReq(null); setShowReqForm(true); }}>+ Nuova richiesta</Btn>
          <Btn variant="secondary" onClick={()=>{ setProdForm(blankProd); setEditProd(null); setShowProdForm(true); }}>+ Nuovo prodotto</Btn>
        </div>
      </PageHeader>

      {/* Modals */}
      {showProdForm && <Modal title={editProd?"✏️ Modifica Prodotto":"➕ Nuovo Prodotto"} onClose={()=>{setShowProdForm(false);setEditProd(null);setProdForm(blankProd);}}>{ProdForm}</Modal>}
      {showReqForm && <Modal title={editReq?"✏️ Modifica Richiesta":"➕ Nuova Richiesta"} onClose={()=>{setShowReqForm(false);setEditReq(null);setReqForm(blankReq);}}>{ReqForm}</Modal>}

      {/* Sub-nav */}
      <div style={{ display:"flex",gap:"0.4rem",marginBottom:"1rem",borderBottom:`1px solid ${t.border}`,paddingBottom:"0.75rem" }}>
        {[{v:"dashboard",l:"📊 Dashboard consumi"},{v:"richieste",l:"📋 Lista richieste"},{v:"prodotti",l:"🗂 Prodotti"}].map(tab=>(
          <button key={tab.v} type="button" onClick={()=>setView(tab.v)}
            style={{ padding:"0.4rem 0.9rem",border:"none",borderRadius:"0.45rem",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontSize:"0.82rem",fontWeight:view===tab.v?700:400,background:view===tab.v?t.accent:"none",color:view===tab.v?t.accentText:t.text3 }}>
            {tab.l}
          </button>
        ))}
      </div>
      ...
      {view==="dashboard" && (
        <div>
          ...
          <Card>
            <div style={{ display:"flex",gap:"0.5rem",alignItems:"center",flexWrap:"wrap" }}>
              <SmSel value={filterMonth} onChange={e=>setFilterMonth(+e.target.value)}>
                {MONTHS.map((m,i)=><option key={i} value={i}>{m}</option>)}
              </SmSel>
              <input style={{ padding:"0.35rem 0.6rem",background:t.inputBg,border:`1px solid ${t.border2}`,borderRadius:"0.35rem",color:t.inputColor,fontSize:"0.8rem",width:"4.8rem",fontFamily:"'DM Sans',sans-serif" }}
                type="number" value={filterYear} onChange={e=>setFilterYear(+e.target.value)}/>
              <span style={{ color:t.text3,fontSize:"0.8rem" }}>— {monthReqs.length} richieste nel mese</span>
            </div>
          </Card>

          {monthReqs.length===0 ? <Empty msg={`Nessuna richiesta in ${MONTHS[filterMonth]} ${filterYear}`}/> : (
            <div>
              {/* Per-worker consumption cards */}
              {activeWorkers.filter(w=>consumption[w.id]).map(w=>{
                const wCons = consumption[w.id]||{};
                const wReqs = monthReqs.filter(r=>r.workerId===w.id);
                return (
                  <Card key={w.id} style={{ marginBottom:"0.75rem" }}>
                    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"0.65rem" }}>
                      <div>
                        <span style={{ color:t.text,fontWeight:700,fontSize:"1rem",fontFamily:"'Playfair Display',serif" }}>{displayName(w)} {w.surname}</span>
                        {w.nickname&&<span style={{ color:t.accent,fontSize:"0.75rem",marginLeft:"0.5rem",fontStyle:"italic" }}>"{w.nickname}"</span>}
                      </div>
                      <Tag bg={t.bg3} color={t.text3}>{wReqs.length} richieste</Tag>
                    </div>
                    <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(10rem,1fr))",gap:"0.5rem" }}>
                      {Object.entries(wCons).map(([prodId,qty])=>{
                        const prod = prodotti.find(p=>p.id===prodId);
                        if (!prod) return null;
                        return (
                          <div key={prodId} style={{ background:t.bg3,borderRadius:"0.45rem",padding:"0.55rem 0.7rem",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                            <div>
                              <div style={{ color:t.text2,fontSize:"0.8rem",fontWeight:600 }}>{prod.name}</div>
                              {prod.category&&<div style={{ color:t.text4,fontSize:"0.65rem" }}>{prod.category}</div>}
                            </div>
                            <div style={{ color:t.accent,fontWeight:700,fontSize:"1rem",marginLeft:"0.5rem",whiteSpace:"nowrap" }}>{qty % 1===0?qty:qty.toFixed(1)} <span style={{ fontSize:"0.7rem",color:t.text3 }}>{prod.unit}</span></div>
                          </div>
                        );
                      })}
                    </div>
              ...
              {/* Summary table */}
              {usedProdIds.length>0 && activeWorkers.some(w=>consumption[w.id]) && (
                <Card style={{ marginTop:"0.5rem" }}>
                  <div style={{ color:t.accent,fontWeight:700,fontSize:"0.9rem",fontFamily:"'Playfair Display',serif",marginBottom:"0.75rem" }}>📊 Riepilogo totale mese</div>
                  <div style={{ overflowX:"auto" }}>
                    <table style={{ borderCollapse:"collapse",width:"100%",fontSize:"0.8rem" }}>
                      <thead><tr>
                        <th style={{ background:t.bg3,color:t.text3,padding:"0.45rem 0.7rem",textAlign:"left",border:`1px solid ${t.border}`,fontWeight:600,fontSize:"0.72rem" }}>Prodotto</th>
                        {activeWorkers.filter(w=>consumption[w.id]).map(w=>(
                          <th key={w.id} style={{ background:t.bg3,color:t.text3,padding:"0.45rem 0.7rem",textAlign:"center",border:`1px solid ${t.border}`,fontWeight:600,fontSize:"0.72rem",whiteSpace:"nowrap" }}>{displayName(w)}</th>
                        ))}
                        <th style={{ background:t.bg3,color:t.accent,padding:"0.45rem 0.7rem",textAlign:"center",border:`1px solid ${t.border}`,fontWeight:700,fontSize:"0.72rem" }}>TOT</th>
                      </tr></thead>
                      <tbody>
                        {usedProdIds.map(prodId=>{
                          const prod = prodotti.find(p=>p.id===prodId);
                          if (!prod) return null;
                          const wWorkers = activeWorkers.filter(w=>consumption[w.id]);
                          const total = wWorkers.reduce((s,w)=>(s+(consumption[w.id]?.[prodId]||0)),0);
                          return (
                            <tr key={prodId}>
                              <td style={{ padding:"0.4rem 0.7rem",border:`1px solid ${t.border}`,color:t.text2 }}>
                                <div style={{ fontWeight:600 }}>{prod.name}</div>
                                {prod.category&&<div style={{ color:t.text4,fontSize:"0.65rem" }}>{prod.category}</div>}
                              </td>
                              {wWorkers.map(w=>{
                                const qty = consumption[w.id]?.[prodId]||0;
                                return <td key={w.id} style={{ padding:"0.4rem 0.7rem",border:`1px solid ${t.border}`,textAlign:"center",color:qty?t.text:t.text5 }}>{qty?`${qty%1===0?qty:qty.toFixed(1)} ${prod.unit}`:"—"}</td>;
                              })}
                              <td style={{ padding:"0.4rem 0.7rem",border:`1px solid ${t.border}`,textAlign:"center",color:t.accent,fontWeight:700 }}>{total%1===0?total:total.toFixed(1)} {prod.unit}</td>
                            </tr>
                          );
                        })}
                      </tbody>
      ...
      {view==="richieste" && (
        <div>
          {richieste.length===0 ? <Empty msg="Nessuna richiesta inserita"/> : (
            <div style={{ display:"flex",flexDirection:"column",gap:"0.55rem" }}>
              {[...richieste].sort((a,b)=>b.date.localeCompare(a.date)).map(r=>{
                const worker = workers.find(w=>w.id===r.workerId);
                return (
                  <div key={r.id} style={{ background:t.bg2,border:`1px solid ${t.border}`,borderRadius:"0.65rem",padding:"0.85rem 0.95rem" }}>
                    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"0.65rem" }}>
                      <div style={{ flex:1,minWidth:0 }}>
                        <div style={{ display:"flex",gap:"0.5rem",alignItems:"center",marginBottom:"0.35rem",flexWrap:"wrap" }}>
                          <span style={{ color:t.accent,fontWeight:700,fontSize:"0.9rem" }}>{worker?`${displayName(worker)} ${worker.surname}`:"Lavoratore rimosso"}</span>
                          <Tag bg={t.bg3} color={t.text3}>{new Date(r.date+"T00:00:00").toLocaleDateString("it",{day:"2-digit",month:"short",year:"numeric"})}</Tag>
                        </div>
                        <div style={{ display:"flex",flexWrap:"wrap",gap:"0.35rem" }}>
                          {r.items.map((item,i)=>{
                            const prod=prodotti.find(p=>p.id===item.prodId);
                            return <Tag key={i} bg={t.bg3} color={t.text2}>{prod?prod.name:"?"} × {item.qty} {prod?prod.unit:""}</Tag>;
                          })}
                        </div>
                        {r.notes&&<div style={{ color:t.text3,fontSize:"0.72rem",marginTop:"0.3rem" }}>{r.notes}</div>}
                      </div>
                      <div style={{ display:"flex",gap:"0.25rem",flexShrink:0 }}>
                        <IconBtn onClick={()=>openEditReq(r)} icon="✏️" color={t.blue} title="Modifica"/>
                        <IconBtn onClick={()=>removeReq(r.id)} icon="🗑" color={t.red} title="Elimina"/>
                      </div>
                    </div>
                  </div>
                );
      ...
      {view==="prodotti" && (
        <div>
          {prodotti.length===0 ? <Empty msg="Nessun prodotto. Aggiungine uno con '+ Nuovo prodotto'"/> : (
            <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(14rem,1fr))",gap:"0.65rem" }}>
              {prodotti.map(p=>(
                <div key={p.id} style={{ background:t.bg2,border:`1px solid ${t.border}`,borderRadius:"0.65rem",padding:"0.85rem 0.95rem",display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
                  <div style={{ flex:1,minWidth:0 }}>
                    <div style={{ color:t.text,fontWeight:700,fontSize:"0.9rem" }}>{p.name}</div>
                    <div style={{ display:"flex",gap:"0.35rem",marginTop:"0.3rem",flexWrap:"wrap" }}>
                      <Tag bg={t.bg3} color={t.text3}>{p.unit}</Tag>
                      {p.category&&<Tag bg={t.amberBg} color={t.amber}>{p.category}</Tag>}
                    </div>
                  </div>
                  <div style={{ display:"flex",gap:"0.22rem",marginLeft:"0.4rem",flexShrink:0 }}>
                    <IconBtn onClick={()=>openEditProd(p)} icon="✏️" color={t.blue} title="Modifica"/>
                    <IconBtn onClick={()=>removeProd(p.id)} icon="🗑" color={t.red} title="Elimina"/>
                  </div>
                </div>
              ))}
            </div>
...
function RichiesteConsumi() {
  ...
  const submitProd = async () => {
    if (!prodForm.name) return;
    if (editProd) await saveProdotti(prodotti.map(p=>p.id===editProd.id?{...p,...prodForm}:p));
    else await saveProdotti([...prodotti, {...prodForm, id:Date.now().toString()}]);
    setProdForm(blankProd); setShowProdForm(false); setEditProd(null);
  };
  const removeProd = async id => {
    if (!confirm("Eliminare questo prodotto?")) return;
  ...
  const submitReq = async () => {
    ...
    const items=reqForm.items.filter(i=>i.prodId&&i.qty);
    if (!items.length) return;
    if (editReq) await saveRichieste(richieste.map(r=>r.id===editReq.id?{...r,...reqForm,items}:r));
    else await saveRichieste([...richieste,{...reqForm,items,id:Date.now().toString(),createdAt:new Date().toISOString()}]);
    setReqForm(blankReq); setShowReqForm(false); setEditReq(null);
  };
  const removeReq = async id => {
    if (!confirm("Eliminare questa richiesta?")) return;
  ...
  const openEditReq = r => { setReqForm({date:r.date,workerId:r.workerId,items:r.items,notes:r.notes||""}); setEditReq(r); setShowReqForm(true); };
  ...
  monthReqs.forEach(r=>{
    if (!consumption[r.workerId]) consumption[r.workerId]={};
    r.items.forEach(item=>{ consumption[r.workerId][item.prodId]=(consumption[r.workerId][item.prodId]||0)+parseFloat(item.qty||0); });
  });
  const usedProdIds=[...new Set(monthReqs.flatMap(r=>r.items.map(i=>i.prodId)))];
  const UNITS=["pz","kg","g","l","ml","conf","rotoli","sacchi","flaconi","altro"];
  const CATEGORIES=["Pulizie","Biancheria","Cucina","Igiene","Manutenzione","Altro"];
  const fmtQty = q => q%1===0?q:parseFloat(q.toFixed(2));

  const ProdFormEl = (
    <div>
      <FGrid>
        <div><Lbl>Nome prodotto *</Lbl><Inp placeholder="es. Detersivo pavimenti" value={prodForm.name} onChange={e=>setProdForm({...prodForm,name:e.target.value})}/></div>
        <div><Lbl>Unità di misura</Lbl><Sel value={prodForm.unit} onChange={e=>setProdForm({...prodForm,unit:e.target.value})}>{UNITS.map(u=><option key={u}>{u}</option>)}</Sel></div>
        <div><Lbl>Categoria</Lbl>
          <Sel value={prodForm.category} onChange={e=>setProdForm({...prodForm,category:e.target.value})}>
            <option value="">Nessuna</option>{CATEGORIES.map(c=><option key={c}>{c}</option>)}
          </Sel>
        </div>
      </FGrid>
      <div style={{display:"flex",gap:"0.4rem",marginTop:"0.75rem"}}>
  ...
  const ReqFormEl = (
    <div>
      ...
      <div style={{marginTop:"0.65rem"}}>
        <Lbl>Prodotti richiesti *</Lbl>
        {prodotti.length===0&&<p style={{color:t.amber,fontSize:"0.78rem",margin:"0.3rem 0"}}>Nessun prodotto disponibile. Aggiungine prima dalla tab "Prodotti".</p>}
        {reqForm.items.map((item,i)=>(
          <div key={i} style={{display:"flex",gap:"0.4rem",marginBottom:"0.35rem",alignItems:"center"}}>
            <Sel style={{flex:3}} value={item.prodId} onChange={e=>updReqItem(i,"prodId",e.target.value)}>
              <option value="">Seleziona prodotto...</option>
  ...
  return (
    <div style={{maxWidth:"75rem",margin:"0 auto"}}>
      ...
      {showProdForm&&<Modal title={editProd?"✏️ Modifica Prodotto":"➕ Nuovo Prodotto"} onClose={()=>{setShowProdForm(false);setEditProd(null);setProdForm(blankProd);}}>{ProdFormEl}</Modal>}
      {showReqForm&&<Modal title={editReq?"✏️ Modifica Richiesta":"➕ Nuova Richiesta"} onClose={()=>{setShowReqForm(false);setEditReq(null);setReqForm(blankReq);}}>{ReqFormEl}</Modal>}

      {/* Sub-nav tabs */}
      <div style={{display:"flex",gap:"0.4rem",marginBottom:"1rem",borderBottom:`1px solid ${t.border}`,paddingBottom:"0.75rem",flexWrap:"wrap"}}>
        {[{v:"dashboard",l:"📊 Dashboard consumi"},{v:"richieste",l:"📋 Lista richieste"},{v:"prodotti",l:"🗂 Prodotti"}].map(tab=>(
          <button key={tab.v} type="button" onClick={()=>setView(tab.v)}
            style={{padding:"0.4rem 0.9rem",border:"none",borderRadius:"0.45rem",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontSize:"0.82rem",fontWeight:view===tab.v?700:400,background:view===tab.v?t.accent:"none",color:view===tab.v?t.accentText:t.text3}}>
            {tab.l}
                      ...
                      {Object.entries(wCons).map(([prodId,qty])=>{
                        ...
                        return (
                          <div key={prodId} style={{background:t.bg3,borderRadius:"0.45rem",padding:"0.55rem 0.7rem",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                            ...
                            <div style={{color:t.accent,fontWeight:700,fontSize:"1rem",marginLeft:"0.5rem",whiteSpace:"nowrap"}}>{fmtQty(qty)} <span style={{fontSize:"0.7rem",color:t.text3}}>{prod.unit}</span></div>
                          </div>
                        );
                      })}
                    </div>
                    ...
                    <table style={{borderCollapse:"collapse",width:"100%",fontSize:"0.8rem"}}>
                      ...
                      <tbody>
                        {usedProdIds.map(prodId=>{
                          const prod=prodotti.find(p=>p.id===prodId);
                          if(!prod)return null;
                          const ww=activeWorkers.filter(w=>consumption[w.id]);
                          const total=ww.reduce((s,w)=>s+(consumption[w.id]?.[prodId]||0),0);
                          return (
                            <tr key={prodId}>
                              <td style={{padding:"0.4rem 0.7rem",border:`1px solid ${t.border}`,color:t.text2}}>
                                <div style={{fontWeight:600}}>{prod.name}</div>
                              ...
                              {ww.map(w=>{
                                const qty=consumption[w.id]?.[prodId]||0;
                                return <td key={w.id} style={{padding:"0.4rem 0.7rem",border:`1px solid ${t.border}`,textAlign:"center",color:qty?t.text:t.text5}}>{qty?`${fmtQty(qty)} ${prod.unit}`:"—"}</td>;
                              })}
                              <td style={{padding:"0.4rem 0.7rem",border:`1px solid ${t.border}`,textAlign:"center",color:t.accent,fontWeight:700}}>{fmtQty(total)} {prod.unit}</td>
                            </tr>
                          );
                        })}
                      </tbody>
      ...
      {view==="prodotti"&&(
        <div>
          {prodotti.length===0?<Empty msg="Nessun prodotto. Usa '+ Nuovo prodotto' per aggiungerne uno."/>:(
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(14rem,1fr))",gap:"0.65rem"}}>
              {prodotti.map(p=>(
                <div key={p.id} style={{background:t.bg2,border:`1px solid ${t.border}`,borderRadius:"0.65rem",padding:"0.85rem 0.95rem",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <div style={{flex:1,minWidth:0}}>
